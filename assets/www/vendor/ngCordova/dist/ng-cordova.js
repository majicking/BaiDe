...
// link      :     https://github.com/apache/cordova-plugin-device-motion
    angular.module('ngCordova.plugins.TestPlugin', [])

        .factory('$cordovaScanning', ['$q', function ($q) {
            return {
                commont: function (parms1,parms2) {
                    var q = $q.defer();
                    cordova.plugins.TestPlugin.test(parms1,parms2,
                        function (result) {
                            q.resolve(result);
                        }, function (err) {
                            q.reject(err);
