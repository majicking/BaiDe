        ...
        return {
            ...
            compile: function(tElement, tAttrs, tTransclude) {
                ...
                var template = angular.element([
                    ...
                    '<ion-item class="item item-divider" id="'+time+'_index_{{letter}}">{{letter}}</ion-item>',
                    '<ion-item ng-repeat="item in items"></ion-item>',
                    '</div>',
                    '</ion-scroll>',
                    '<ul class="ion_alpha_sidebar">',
                    '<li ng-click="alphaScrollGoToList($event,\''+time+'_index\')" ng-repeat="letter in alphabet | orderBy: letter">{{ letter }}</li>',
                    '</ul>',
                    '</ion-list>'
                ].join(''));

                var barHeader = $document[0].body.querySelector('.bar-header');
                var headerHeight = 0;
                if(barHeader){
                    headerHeight = barHeader.offsetHeight;
                }
                ...
                var contentHeight = windowHeight - headerHeight - subHeaderHeight;
                if(tAttrs.ngClick){
                    angular.element(template.find('ion-item')[1]).attr('ng-click', tAttrs.ngClick);
                    tAttrs.ngClick = null;
                }
                if(tAttrs.ngClass){
                    angular.element(template.find('ion-item')[1]).addClass(tAttrs.ngClass);
                    tAttrs.ngClass = null;
                }
                ...
                return function(scope, element, attrs, ngModel) {
                    ...
                    ngModel.$render = function() {
                        ...
                        scope.alphaScrollGoToList = function($event,id) {
                            $location.hash(id+'_'+$event.currentTarget.innerText);
