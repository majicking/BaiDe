(function () {
    ...
    angular.module('app').controller('RestsTransferCtrl', RestsTransferCtrl);
    RestsTransferCtrl.$inject = ['$rootScope', '$scope', '$sessionStorage', '$ionicLoading', '$ionicPopup', '$ionicNativeTransitions', 'AccountService', 'UserService', '$stateParams'];
    function RestsTransferCtrl($rootScope, $scope, $sessionStorage, $ionicLoading, $ionicPopup, $ionicNativeTransitions, AccountService, UserService, $stateParams) {
        var vm = this;
        ...
        function confirm() {
            ...
            }else if(vm.input.money < 50){
                $scope.$emit('alertWarning', '转出数量低于最低额');
            } else if (vm.input.money % 50 != 0 ) {
                $scope.$emit('alertWarning', '转账额度必须是50的整数倍');
