(function () {
    ...
    angular.module('app').controller('ApplyForCtrl', ApplyForCtrl);
    ApplyForCtrl.$inject = ['$rootScope', '$sessionStorage', '$scope', '$ionicLoading', '$ionicNativeTransitions', 'AccountService', '$stateParams'];

    function ApplyForCtrl($rootScope, $sessionStorage, $scope, $ionicLoading, $ionicNativeTransitions, AccountService, $stateParams) {
        var vm = this;
        ...
        function getWithdrawInfo() {
            $ionicLoading.show();
            AccountService.declarationCoinInfo(
                vm.userId,
                vm.subAccountType,
                '0028'
        ...
        function confirm() {
            if (!vm.model.withdrawType) {
                $scope.$emit('alertWarning', '此用户不能申请，详情请联系管理员');
            }
            // else if(vm.model.amount < vm.input.money){
            //     $scope.$emit('alertWarning', '申请额度超限');
            // }
            // else if(vm.model.withdrawLimit != null
            //     && vm.input.money < vm.model.withdrawLimit){
            //     $scope.$emit('alertWarning', '申请额度低于最低额');
            // }
            else if (vm.input.money < 100) {
                $scope.$emit('alertWarning', '申请数量低于最低额');
            }
            else if (vm.input.money % 100 != 0) {
                $scope.$emit('alertWarning', '申请额度必须是100的整数倍');
                ...
                AccountService.declarationCoin(
                    vm.input
                ).success(function (data) {
                    $ionicNativeTransitions.stateGo('tab.account-applyFor-result', {
                        id: data,
