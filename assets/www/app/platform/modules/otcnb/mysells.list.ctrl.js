...
(function () {
    ...
    /* Controllers */
    angular.module('app').controller('OtcMySellsListNbCtrl', OtcMySellsListNbCtrl);

    OtcMySellsListNbCtrl.$inject = ['$stateParams', '$rootScope', '$scope', '$state', '$ionicPopover', 'OtcNbService', '$ionicLoading', '$ionicNativeTransitions'];
    function OtcMySellsListNbCtrl($stateParams, $rootScope, $scope, $state, $ionicPopover, OtcNbService, $ionicLoading, $ionicNativeTransitions) {
        var vm = this;
        ...
        function doRefresh() {
            ...
            OtcNbService.getOtcSellList(1, vm.page.itemsPerPage, vm.criteria)
                .success(function (data) {
                    ...
                    vm.items = [];
                    if (vm.model.otcSellNbModelList.length == 0) {
                        vm.hasMoreData = false;
                        vm.hasData = false;
                    } else {
                        angular.forEach(vm.model.otcSellNbModelList, function (item) {
                            if(item.status == 0){
                                item.status_str = '挂单中';
                        ...
                        vm.page.currentPage++;
                        if (vm.model.otcSellNbModelList.length < vm.page.itemsPerPage) {
                            vm.hasMoreData = false;
                            vm.hasData = false;
                        } else {
                            vm.hasMoreData = true;
